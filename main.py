...
from feature_extractor import directory_path
from workflow_analyser import analyze_repository_workflow, functionality_extractor
...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "gemini Api key"
    ...
    try:
        ...
        if "Could not resolve host: github.com" in error_message:
            ...
            if not check_internet_connection():
                ...
                st.error("Internet connection is available, but GitHub cannot be reached. This might be a temporary issue with GitHub or your DNS settings.")
            st.info("Suggestions:")
...
# ... (keep all the existing functions like get_file_structure, get_file_content, etc.)
...
def analyze_directory_workflow(directory_path):
    ...
    structure = get_file_structure(directory_path)
    file_summaries = []
...
def display_directory_contents(directory):
    ...
    dirs, files = list_files_and_dirs(directory)
    ...
    if analyze_checkbox:
        st.write(analyze_directory_workflow(directory))
    ...
    try:
        ...
        if analyze_checkbox:
            ...
            with c2.container(height=1000):
                st.write(analyze_file_workflow(file_path))
    except Exception as e:
        st.error(f"Error reading file {file_path}: {e}")


# Keep your existing functions: get_file_content, get_file_content_summary,
# analyze_directory_workflow, analyze_file_workflow, list_files_and_dirs

# New function to handle chat queries
...
class RepositoryAnalyzer:
    ...
    def build_repository_overview(self):
        ...
        structure = get_file_structure(self.root_directory)
    ...
    def _analyze_directory(self, dir_path, relative_path):
        ...
        structure = get_file_structure(dir_path)
        files_content = ""
...
def process_query(query, analyzer):
    """Process a query using the repository analyzer."""
    if not analyzer.repo_overview:
        return "Repository overview not yet built. Please wait."
    ...
    # 3. Generate answer using detailed analyses
    answer_prompt = f"""Based on these analyses:
...
#class RepositoryAnalyzer:
    # ... (keep the existing RepositoryAnalyzer class implementation)
...
def main():
    ...
    if st.sidebar.button("Add Repository"):
        if repo_url and repo_name:
            ...
            if clone_repository(repo_url, local_path):
                st.session_state.repositories[repo_name] = RepositoryAnalyzer(local_path)
        ...
        if st.sidebar.checkbox("Analyze Repository"):
            ...
            if st.sidebar.button("Run Workflow Analysis"):
                with st.spinner(f"Analyzing repository {selected_repo}..."):
                    root_directory = st.session_state.repositories[selected_repo].root_directory
                    st.session_state.analysis_results[selected_repo] = analyze_repository_workflow(root_directory)
    ...
    with tab1:
        if selected_repo and st.session_state.get('show_file_browser', False):
            display_directory_contents(st.session_state.current_path)
            if st.session_state.selected_file:
                display_file_contents(st.session_state.selected_file)
        else:
    ...
    with tab2:
        ...
        # Chat mode selection
        chat_mode = st.radio("Select Chat Mode", ["General Chat", "Repository-Specific Chat"])
        ...
        if chat_mode == "General Chat":
            ...
            if prompt := st.chat_input("Ask about all your repositories"):
                ...
                with st.chat_message("assistant"):
                    response = process_query(prompt, st.session_state.repositories)
            ...
            if selected_repo:
                ...
                if prompt := st.chat_input(f"Ask about {selected_repo}"):
                    ...
                    with st.chat_message("assistant"):
                        ...
                        response = process_single_repo_query(prompt, repo_analyzer)
                        st.write(response['answer'])
